...
fn main() {
    ...
    let gate_input = spec.add_node(GATE_INPUT_SIZE, ActivationFn::Tanh);
    let hidden = spec.add_node(HIDDEN_SIZE, ActivationFn::Tanh);
    let gate_output = spec.add_node(GATE_OUTPUT_SIZE, ActivationFn::Tanh);

    spec.add_edge(gate_input, hidden);
    spec.add_edge(hidden, gate_output);
    ...
    let initial_error = test_it(gate_input, gate_output, &mut xor, &mut rng);
    println!("Pre learning error: {}", initial_error);

    println!("LEARNING");
    learn_it(gate_input, gate_output, &mut xor, &mut rng);

    println!("POST LEARNING");
    let final_error = test_it(gate_input, gate_output, &mut xor, &mut rng);
