...
trait PCNAlgorithms<Id: Eq + Ord + Clone> {
    fn train_supervised<I: IntoIterator<Item = LabeledData<Id>>, R: Rng>(&mut self, rng: &mut R, batch: I, alpha: f64, gamma: f64, inference_steps: usize);
    fn train_unsupervised<I: IntoIterator<Item = Data<Id>>>(&mut self, batch: I);
    fn evaluate<I: IntoIterator<Item = LabeledData<Id>>, R: Rng>(&mut self, rng: &mut R, batch: I, gamma: f64, inference_steps: usize) -> f64;
    fn infer<R: Rng>(&mut self, gamma: f64, inference_steps: usize, data: Data<Id>, output: &mut BTreeMap<Id, Vec<f64>>);
    fn new_output_map<I: IntoIterator<Item = Id>>(&self, output_nodes: I) -> BTreeMap<Id, Vec<f64>>;
}
